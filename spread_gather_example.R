...
# So take what you read here with a grain of salt.  If you want to learn more this is a great resource
# the whole book is free 

"https://r4ds.had.co.nz/tidy-data.html"

# instead of pretending to know what makes data tidy, I will cover moving your data between 'long' and 'wide' and why each is valuable

# CD3 <- read_csv('../fsep_meetup/CD3pos.csv')
...
### lets clean up the '/' in the cell type names

CD3.long <- CD3.long %>% mutate(cell_type=gsub('/','', cell_type))


CD3.long %>% ggplot(aes(x=tissue, y=percent_CD3, fill=treatment)) + geom_boxplot() + facet_wrap(~cell_type, scales = 'free')
...
# only cell types with a 
CD3_sigs2 <- CD3_tests %>% filter(wilcox_p < 0.05 & t_p < 0.05)

# only show cell types with some a wilcox pvalue > 0.05
...
CD3_summary <- CD3.long %>% group_by(tissue, treatment, cell_type) %>% 
                  summarise(avg=mean(percent_CD3), 
                    ...
                    num_obs= n(), 
                    std_err=std_dev/sqrt(num_obs)) %>% filter(cell_type %in% CD3_sigs2$cell_type)
...
# if you have a favorite cell type...

CD3_summary %>% filter(cell_type == 'CD3+CD4+CD8a-FoxP3+CD25+' & tissue == 'cec') %>% 
...
#### why would you ever want data in a wide format?

# Adding a grouping variable and then going from long to wide

CD3.wide <- CD3.long %>% mutate(set=paste(tissue, treatment,sep = '_')) %>% 
  spread(key = cell_type, value = percent_CD3)

#### some tools require data in that format ####
